package com.wazzgroup.penagihanwifi.psb;
...
public class PsbActivity extends AppCompatActivity {
...
//        loadSpinnerodp(spinnerodp); // Panggil fungsi tanpa perlu tulis URL
        ...
        editTanggal.setOnClickListener(v -> {
            ...
            DatePickerDialog datePicker = new DatePickerDialog(PsbActivity.this,
...
//    }
    ...
    private void ambilDataODPdanTampilkanMap(double latUser, double lonUser) {
        ...
        client.newCall(request).enqueue(new Callback() {
            @Override
            public void onFailure(Call call, IOException e) {
                runOnUiThread(() -> Toast.makeText(PsbActivity.this, "Gagal ambil data ODP: " + e.getMessage(), Toast.LENGTH_SHORT).show());
            }
            ...
            public void onResponse(Call call, Response response) throws IOException {
                if (response.isSuccessful()) {
                    ...
                    try {
                        ...
                        runOnUiThread(() -> Toast.makeText(PsbActivity.this, "Format JSON salah: " + e.getMessage(), Toast.LENGTH_SHORT).show());
                    }
                }
            }
        });
...
//        }
        ...
        client.newCall(request).enqueue(new Callback() {
            @Override
            public void onFailure(Call call, IOException e) {
                runOnUiThread(() -> Toast.makeText(PsbActivity.this, "Gagal: " + e.getMessage(), Toast.LENGTH_SHORT).show());
            }
                ...
                runOnUiThread(() -> {
                    try {
                        ...
                        if (status.equalsIgnoreCase("success")) {
                            ...
                            Toast.makeText(PsbActivity.this, "Gagal: " + message, Toast.LENGTH_LONG).show();
                        }
                    } catch (JSONException e) {
                        Toast.makeText(PsbActivity.this, "Respon tidak valid dari server", Toast.LENGTH_LONG).show();
                    }
